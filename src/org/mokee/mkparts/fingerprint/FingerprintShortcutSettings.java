...
public class FingerprintShortcutSettings extends SettingsPreferenceFragment {
    ...
    public boolean onPreferenceTreeClick(Preference preference) {
        ...
        final int fingerId = pref.getFingerprint().getFingerId();
        final Target target = mShortcuts.get(fingerId);

        if (target == null) {
            startEditorActivity(fingerId);
        } else {
            showEditDialog(target, pref.getFingerprint());
        }
    ...
    private void showEditDialog(Target target, Fingerprint fingerprint) {
        final int fingerId = fingerprint.getFingerId();
        new AlertDialog.Builder(getContext())
                ...
                .setNeutralButton(R.string.fingerprint_shortcut_delete,
                        new DialogInterface.OnClickListener() {
                            @Override
                            public void onClick(DialogInterface dialog, int which) {
                                mShortcutManager.removeShortcut(fingerId);
                                updatePreferences();
                            }
                        })
                .show();
    ...
    private void startEditorActivity(int fingerId) {
        final Intent intent = new Intent(getContext(), FingerprintShortcutEditor.class);
        intent.putExtra(FingerprintShortcutEditor.EXTRA_FINGER_ID, fingerId);
